...
"%PYTHON_EXE%" -m pip install --upgrade "flask[async]" "uvicorn[standard]" playwright requests beautifulsoup4 lxml pandas >NUL
if errorlevel 1 goto :error

echo [INFO] Verificando navegadores de Playwright (chromium)...
"%PYTHON_EXE%" -m playwright install chromium >NUL
if errorlevel 1 goto :error
